...
export default function RegisterUserInfoForm() {
  const createNewUserWithEmailAndPassword = async () => {
    try {
      const newUserFirebase = createUserWithEmailAndPassword(
        auth,
        email,
        password
      );
    ...
    <>
      ...
      >
        <div className="space-y-5 w-96">
          ...
          <div>
            <div className="input-group group focus-within:p-[0.70rem] focus-within:border-2 focus-within:border-blue-600 w-96">
              <input
                type="text"
                id="Name"
          ...
          <div className="input-items">
            ...
            {!isEmailAuth
              ? errors.phone?.type === "required" && (
                  <TextAlert alert="Phone is required" />
                )
              : errors.email?.type === "required" && (
                  <TextAlert alert="Email is required" />
                )}
            {!isEmailAuth
              ? errors.phone?.type === "maxLength" && (
                  <TextAlert alert="Phone is too long" />
                )
              : errors.email?.type === "maxLength" && (
                  <TextAlert alert="Email is too long" />
                )}
            {!isEmailAuth
              ? errors.phone?.type === "minLength" && (
                  <TextAlert alert="Phone is too short" />
                )
              : errors.email?.type === "minLength" && (
                  <TextAlert alert="Email is too short" />
                )}
          ...
          <button className="bg-white hover:bg-slate-200 text-slate-800 flex items-center justify-center rounded-full py-1 h-10 w-96 px-4 mb-1 shadow-md text-md font-bold">
            Next
          </button>
        </div>
      </form>
